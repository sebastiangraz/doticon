...
Multiple states exist (`StateKey = "dormant" | "thinking" | "loading" | "dev"`); more can be added by registering one entry in `buildStates` plus layout/opacity definitions. The `dev` state is only visible when `isDevStateEnabled` is true (localhost / `VITE_ENABLE_DEV_STATE=true`); when disabled, `state="dev"` falls back to `"dormant"`.
